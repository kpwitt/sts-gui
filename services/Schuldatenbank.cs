        ...
        try {
            ...
            sqliteCmd.CommandText = """
                                    CREATE TABLE IF NOT EXISTS
                                        ...
                                        [has_m365] INTEGER DEFAULT 1 NOT NULL,
                                        [aktiv] BOOLEAN NOT NULL DEFAULT TRUE,
                                        [seriennummer] NVARCHAR(64) NOT NULL DEFAULT '',
...
/*
    ...
    /// <param name="bemerkung"></param>
    public Task Addlehrkraft(int id, string vorname, string nachname, string kuerzel, string mail,
    ...
    /// <returns></returns>
    public Task Addlehrkraft(Lehrkraft lehrkraft) {
        try {
            if (GibtEsLehrkraft(lehrkraft.ID)) return Task.CompletedTask;
    ...
    public Task AddSchuelerIn(int id, string vorname, string nachname, string mail, string klasse,
        ...
        try {
            if (GibtEsSchueler(id)) return Task.CompletedTask;
            seriennummer = string.IsNullOrEmpty(seriennummer) ? "" : seriennummer;
            var sqliteCmd = _sqliteConn.CreateCommand();
            sqliteCmd.CommandText =
                "INSERT OR IGNORE INTO schueler (id, vorname, nachname, mail, klasse, nutzername, aixmail, zweitaccount, zweitmail, seriennummer, has_m365, has_jamf, aktiv,bemerkung)" +
    ...
    /// <returns></returns>
    public Task AddSchuelerIn(SuS schuelerin) {
        try {
            if (GibtEsSchueler(schuelerin.ID)) return Task.CompletedTask;
            ...
            if (!parameters.ExpandFiles) {
                ...
                if (parameters.TargetSystems.Contains('m')) {
                    ...
                    await File.WriteAllLinesAsync($"{parameters.Folder}/mdl_nutzer.csv",
                        ...
                        Encoding.UTF8);
                    await File.WriteAllLinesAsync($"{parameters.Folder}/lkmdl_kurse.csv",
                        ausgabeMoodleKurse.Distinct().ToList(),
                        Encoding.UTF8);
                    await File.WriteAllLinesAsync($"{parameters.Folder}/lkmdl_nutzer.csv",
                        ausgabeMoodleUser.Distinct().ToList(),
                        Encoding.UTF8);
                }
            ...
            foreach (var kurs in GetLKKurseVonLuL(lt.ID).Result) {
                ...
                if (kurs.Bezeichnung.Contains("Jahrgangsstufenkonferenz")) {
                    ...
                    var rolle = stufenleitungen.Contains(lt) ||
                        ...
                        ? "editingteacher"
                        : "student";
                    ausgabeMoodleLKEinschreibungen.Add($"add,{rolle},{lt.ID},{kurs.Bezeichnung}{kurs.Suffix}");
                }
                else {
                    ausgabeMoodleLKEinschreibungen.Add($"add,editingteacher,{lt.ID},{kurs.Bezeichnung}{kurs.Suffix}");
                }
            }

            ausgabeMoodleLKEinschreibungen.AddRange(lt.Fakultas.Split(',')
    ...
    private void ExportKurse(ExportParameters parameters) {
        ...
        Tooling.AppendToFile($"{parameters.Folder}/mdl_kurse.csv", ausgabeMoodleKurse);
        Tooling.AppendToFile($"{parameters.Folder}/lkmdl_kurse.csv", ausgabeMoodleLKKurse);
    }
    ...
    private Task<string> GetKursSuffix() {
        try {
            return Task.FromResult(GetSettings().Result.Kurssuffix);
        }
        catch (Exception exception) {
    ...
    public Task<ReadOnlyCollection<LogEintrag>> GetLog() {
        try {
            ...
            _logpath = log.Settings.LogFilename =
                _dbpath == ":memory:"
                    ? Path.Combine(Path.GetTempPath(), Path.ChangeExtension($"StS_{Guid.NewGuid()}_tmp", "log"))
                    : _dbpath.Replace("sqlite", "log");
            return Task.FromResult(new ReadOnlyCollection<LogEintrag>(logentries));
        }
        catch (Exception exception) {
            return Task.FromException<ReadOnlyCollection<LogEintrag>>(exception);
        }
    }
    ...
    public async Task<SuS> GetSchueler(int id) {
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText =
            "SELECT id,nachname,vorname,mail,klasse,nutzername,aixmail,zweitaccount,zweitmail, has_m365, aktiv, seriennummer, has_jamf, bemerkung FROM schueler WHERE id = $id;";
        sqliteCmd.Parameters.AddWithValue("$id", id);
        var sqliteDatareader = await sqliteCmd.ExecuteReaderAsync();
        SuS schuelerin = new();
        while (sqliteDatareader.Read()) {
    ...
    public async Task<List<SuS>> GetSchueler(string vorname, string nachname) {
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText =
            "SELECT id,nachname,vorname,mail,klasse,nutzername,aixmail,zweitaccount,zweitmail, has_m365, aktiv, seriennummer, has_jamf,bemerkung FROM schueler WHERE vorname LIKE $vorname AND nachname = $nachname;";
        sqliteCmd.Parameters.AddWithValue("$vorname", vorname);
        sqliteCmd.Parameters.AddWithValue("$nachname", nachname);
        var sqliteDatareader = await sqliteCmd.ExecuteReaderAsync();
    ...
    private async Task<SuS> GetSchueler(string vorname, string nachname, string klasse) {
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText =
            "SELECT id,nachname,vorname,mail,klasse,nutzername,aixmail,zweitaccount,zweitmail, has_m365, aktiv, seriennummer, has_jamf,bemerkung FROM schueler WHERE vorname = $vorname AND nachname = $nachname AND klasse = $klasse;";
        sqliteCmd.Parameters.AddWithValue("$vorname", vorname);
        sqliteCmd.Parameters.AddWithValue("$nachname", nachname);
        sqliteCmd.Parameters.AddWithValue("$klasse", klasse);
        var sqliteDatareader = await sqliteCmd.ExecuteReaderAsync();
    ...
    public async Task<List<SuS>> GetSchueler(string name) {
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText =
            "SELECT id,nachname,vorname,mail,klasse,nutzername,aixmail,zweitaccount,zweitmail, has_m365, aktiv, seriennummer, has_jamf,bemerkung FROM schueler WHERE CONCAT(vorname, ' ', nachname)== $name;";
    ...
    public async Task<ReadOnlyCollection<SuS>> GetSchuelerListe() {
        ...
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText =
            "SELECT id,nachname,vorname,mail,klasse,nutzername,aixmail,zweitaccount,zweitmail,has_m365, aktiv, seriennummer,bemerkung, has_jamf FROM schueler;";
        var sqliteDatareader = await sqliteCmd.ExecuteReaderAsync();
        while (sqliteDatareader.Read()) {
            SuS schuelerin = new() {
                ID = sqliteDatareader.GetInt32(0),
    ...
    private Task<List<Lehrkraft>> GetOberstufenleitung(string stufe) {
        try {
            ...
            return Task.FromException<List<Lehrkraft>>(exception);
        }
    }
    ...
    /// </summary>
    /// <param name="kbez"></param>
    public Task RemoveK(string kbez) {
        try {
    ...
    /// </summary>
    /// <param name="sid"></param>
    public Task RemoveS(int sid) {
        try {
            if (sid <= 0) return Task.CompletedTask;
            var sqliteCmd = _sqliteConn.CreateCommand();
    ...
    public Task StartTransaction() {
        try {
            if (_activeTransaction) return Task.CompletedTask;
    ...
    /// <param name="istkurs"></param>
    /// <param name="bemerkung"></param>
    public Task UpdateKurs(string bez, string fach, string klasse, string stufe, string suffix, int istkurs,
        string bemerkung) {
        try {
    ...
    /// <param name="seriennummer"></param>
    /// <param name="bemerkung"></param>
    public Task UpdateLehrkraft(int id, string vorname, string nachname, string kuerzel, string mail,
        string fakultas, string pwtemp, string favo, string sfavo, string seriennummer, string bemerkung) {
        try {
            if (string.IsNullOrEmpty(kuerzel) || id <= 0) return Task.CompletedTask;
            var sqliteCmd = _sqliteConn.CreateCommand();
            sqliteCmd.CommandText =
    ...
    public async Task UpdateSchueler(int id, string vorname, string nachname, string mail, string klasse,
        ...
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText =
            "UPDATE schueler SET nachname=$nachname, vorname=$vorname, mail=$mail, klasse=$klasse, nutzername=$nutzername, aixmail=$aixmail, zweitaccount = $zweitaccount, zweitmail=$zweitmail, has_m365=$hasM365, aktiv=$aktiv, seriennummer=$seriennummer, has_jamf = $jamf,bemerkung=$bemerkung WHERE id=$id;";
        sqliteCmd.Parameters.AddWithValue("$id", id);
        sqliteCmd.Parameters.AddWithValue("$vorname", vorname);
        sqliteCmd.Parameters.AddWithValue("$nachname", nachname);
        sqliteCmd.Parameters.AddWithValue("$mail", mail);
    ...
    public async Task<ReadOnlyCollection<int>> GetM365Blacklist() {
        List<int> ids = [];
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText = "SELECT id FROM schueler WHERE has_m365 = 0;";
        var sqliteDatareader = await sqliteCmd.ExecuteReaderAsync();
        while (sqliteDatareader.Read()) {
    ...
    public async void SetM365(int susid, int has_m365) {
        if (susid <= 0) return;
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText = "UPDATE schueler SET has_m365 = $has_m365 WHERE id = $susid;";
    ...
    private Task SetZweitAccount(int id, int pStatus) {
        try {
    ...
    public Task<List<Lehrkraft>> GetFavos() {
        try {
            return Task.FromResult(GetLehrkraftListe().Result
                .Where(l => !string.IsNullOrEmpty(l.Favo) || !string.IsNullOrEmpty(l.SFavo))
                .ToList());
        }
        catch (Exception exception) {
            return Task.FromException<List<Lehrkraft>>(exception);
    ...
    public void SetJAMF(int susid, int i) {
        var sqliteCmd = _sqliteConn.CreateCommand();
        sqliteCmd.CommandText = "UPDATE schueler SET has_jamf = $jamf WHERE id = $susid;";
        sqliteCmd.Parameters.AddWithValue("$susid", susid);
                ...
                case ChangeKind.add:
                    switch (change.person) {
                        case ChangePerson.SuS:
                            ...
                            moodle_einschreibungen.Add(
                                $"add,student,{change.id},{change.kurs.Bezeichnung}{GetKursSuffix().Result}");
                            break;
                        case ChangePerson.LuL:
                            await AddLtoK(change.id, change.kurs.Bezeichnung);
                            lulidliste.Add(change.id);
                            moodle_einschreibungen.Add(
                                $"add,editingteacher,{change.id},{change.kurs.Bezeichnung}{GetKursSuffix().Result}");
                            break;
                        default:
                            AddLogMessage(new LogEintrag {
                                Eintragsdatum = DateTime.Now,
                ...
                case ChangeKind.del:
                    switch (change.person) {
                        case ChangePerson.SuS:
                            ...
                            moodle_einschreibungen.Add(
                                $"del,student,{change.id},{change.kurs.Bezeichnung}{GetKursSuffix().Result}");
                            break;
                        case ChangePerson.LuL:
                            await RemoveLfromK(change.id, change.kurs.Bezeichnung);
                            moodle_einschreibungen.Add(
                                $"del,editingteacher,{change.id},{change.kurs.Bezeichnung}{GetKursSuffix().Result}");
                            break;
                        default:
                            AddLogMessage(new LogEintrag {
                                Eintragsdatum = DateTime.Now,
